...
import com.appzoneltd.lastmile.microservice.ondemandworkflow.model.RegistrationModel;
import com.appzoneltd.lastmile.microservice.ondemandworkflow.workflowDao.RegistrationRepository;
...
public class JobOrdersService {
	...
	public void assignJobOrderForDriver(Long driverId, JobOrder jobOrder, Long hubId) {
		/// Getting Model
		...
		if (registrationModel != null) {
			...
			if ("FREELANCER_DRIVER".equalsIgnoreCase(registrationModel.getUserType())) {
				FreelancerDriverEntity freelancerDriverEntity = freelancerDriverJpaRepository.findOne(driverId);
				freelancerDriverEntity.setBuildingId(hubId);
				freelancerDriverJpaRepository.save(freelancerDriverEntity);
	...
	public void changeRequestStatus(Long driverId, Long requestId, String status) {
		...
		if (registrationModel != null) {
			...
			if ("FREELANCER_DRIVER".equalsIgnoreCase(registrationModel.getUserType())) {
				if (status.equalsIgnoreCase(ChangePackageStatusEnum.PICKEDUP.getPackageStatus())) {
					FreelancerDriverEntity freelancerDriverEntity = freelancerDriverJpaRepository.findOne(driverId);
					freelancerDriverEntity.setBuildingId(null);
					freelancerDriverJpaRepository.save(freelancerDriverEntity);

					registrationModel.setHubId(null);
				}
			}
		...
		for (N1qlQueryRow n1qlQueryRow : allRows) {
			...
			for (JobOrder jobOrder : registrationModel.getJobOrders()) {
				if (jobOrder.getJobOrderId().equals(requestId)) {
					...
					if ("FREELANCER_DRIVER".equalsIgnoreCase(registrationModel.getUserType())) {
						FreelancerDriverEntity freelancerDriverEntity = freelancerDriverJpaRepository
								.findOne(registrationModel.getDriverId());
						freelancerDriverEntity.setBuildingId(null);
						freelancerDriverJpaRepository.save(freelancerDriverEntity);
						registrationModel.setHubId(null);
					}
