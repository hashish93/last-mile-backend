...
import com.appzoneltd.lastmile.microservice.manualdistribution.entity.ActiveVehicleWithWeight;
...
import com.appzoneltd.lastmile.microservice.manualdistribution.service.automaticservice.AutomaticDistributionService;
import com.appzoneltd.lastmile.microservice.manualdistribution.service.saveplan.SavePlanServiceImpl;
...
public class AutomaticDistributionServiceImp implements AutomaticDistributionService {
    ...
    @Autowired
    private SavePlanServiceImpl savePlanService;
    @Autowired
    ...
    private List<JobOrderTodayDto> getPickupJobToday() throws Exception {
        return manualDistributionService.getJobOrderForToday();
    }
    ...
    private boolean isDeliveryVehicleExists() throws Exception {
        int DeliveryJobsSize = getPickupJobToday().size();
    ...
    private BigDecimal getPickupTotalCapacity() throws Exception {
        List<JobOrderTodayDto> pickupJobs = getPickupJobToday();
    ...
    private boolean isVehicleMatchType(ActiveVehicle vehicle, JobOrderTodayDto job) {
        return job.getJobType().equalsIgnoreCase(vehicle.getPurpose()) || vehicle.getPurpose().equalsIgnoreCase("pickup_delivery");
    }
