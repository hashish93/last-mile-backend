...
import com.appzoneltd.lastmile.microservice.pushnotification.firebase.Result.Builder;
...
import static com.appzoneltd.lastmile.microservice.pushnotification.firebase.Constants.*;
...
public class FCMSender {
    ...
    public Result sendNoRetry(Message message, String registrationId) throws IOException {
        ...
        for (Entry<String, Object> entry : message.getData().entrySet()) {
            ...
            addParameter(body, key, URLEncoder.encode(JSONValue.toJSONString(value), UTF8));
        }
    ...
    public MulticastResult sendNoRetry(Message message, List<String> registrationIds) throws IOException {
        ...
        Map<String, Object> notificationPayload = message.getNotification();
        if (!payload.isEmpty()) {
            jsonRequest.put(JSON_PAYLOAD, payload);
        }

        if (!notificationPayload.isEmpty()) {
            jsonRequest.put(JSON_NOTIFICATION_PAYLOAD, notificationPayload);
        }

        String requestBody = JSONValue.toJSONString(jsonRequest);
        logger.finest("JSON request: " + requestBody);
        HttpURLConnection conn = post(FCM_SEND_ENDPOINT, "application/json", requestBody);
        int status = conn.getResponseCode();
        String responseBody;
        if (status != 200) {
