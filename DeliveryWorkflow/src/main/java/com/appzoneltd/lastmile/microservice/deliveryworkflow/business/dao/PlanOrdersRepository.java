...
public interface PlanOrdersRepository extends PagingAndSortingRepository<PlanOrderEntity, Long> {
	...
	// @Query(value = "SELECT o FROM PlanOrderEntity o WHERE o.plan.id=:id ORDER
	// BY o.id ASC ")
	// List<PlanOrderEntity> findByPlanIdGroupByActiveVehicle(@Param("id") long
	// planId);
	...
	List<Long> getOrderAssignedToActiveVehicle(@Param("activeVehicleId") Long activeVehicleId,
			@Param("planId") Long planId);

	@Query("SELECT p FROM PlanOrderEntity p WHERE p.orderType='DELIVERY' AND p.orderId=:id AND Date(p.created) = Date(now())")
	PlanOrderEntity findByDeliveryId(@Param("id") Long deliveryId);
