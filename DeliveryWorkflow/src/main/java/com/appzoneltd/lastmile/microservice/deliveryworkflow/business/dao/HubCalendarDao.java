...
public interface HubCalendarDao extends PagingAndSortingRepository<HubCalendarEntity, Long> {
	...
	@Query("SELECT DISTINCT h FROM HubCalendarEntity h inner join h.building b inner join h.calendar c WHERE b.buildingId=:hubId AND c.dayname = :dayname")
	public HubCalendarEntity findHubCalendarwithCalendarDayName(@Param("hubId") Long hubId,@Param("dayname") String dayname);
